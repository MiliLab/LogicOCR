...
def _get_args():
    ...
    parser.add_argument("--model_path", type=str, default="AIDC-AI/Ovis2-4B")
    ...
    for item in tqdm(data):
        ...
        if input_modal == "image-text":
            ...
            if args.answer_directly:
                query = f"<image>\n{item['question']} Directly answer the question using a single word or phrase without explanation."
            else:
                query = f"<image>\n{item['question']} The last line of your response should be of the following format: 'Answer: YOUR_ANSWER' where YOUR_ANSWER is the final answer. Think step by step before answering."
        ...
        output_ids = model.generate(input_ids, pixel_values=pixel_values, attention_mask=attention_mask, **generate_kwargs)[0]
