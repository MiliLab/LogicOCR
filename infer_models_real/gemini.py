...
def _get_args():
    ...
    parser.add_argument("--output_filename", type=str, default="gemini")
...
def get_response(data, args):
    ...
    for _ in range(retry_count):
        try:
                ...
                messages=[
                    {
                        "role": "user",
                        "content": [
                            {"type": "text", "text": data["question"] + " The last line of your response should be of the following format: 'Answer: YOUR_ANSWER' where YOUR_ANSWER is the final answer. Think step by step before answering."},
                            {
                                "type": "image_url",
            ...
            if args.verbose:
                print(f'{content}\nCompletion tokens: {completion_tokens}, GT: {data["solution"]}')
