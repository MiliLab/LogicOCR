...
def _get_args():
    ...
    parser.add_argument("--output_filename", type=str, default="gpt4o")
...
def get_response(data, args):
    ...
    for _ in range(retry_count):
        try:
                ...
                messages=[
                    {
                        "role": "user",
                        "content": [
                            {"type": "text", "text": responsed_data["question"] + " The last line of your response should be of the following format: 'Answer: YOUR_ANSWER' where YOUR_ANSWER is the final answer. Think step by step before answering."},
                            {
                                "type": "image_url",
            ...
            responsed_data["output_tokens"] = response.usage.completion_tokens
            
            if args.verbose:
                print(f'Prediction: {content}\nGT: {responsed_data["solution"]}')
